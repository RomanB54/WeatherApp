...
describe('View Class', () => {
  ...
  it('addWeatherTemplate should replace placeholders correctly', () => {
    ...
    const weatherObject: IWeatherObject =  { city: 'Moscow', temperature: 5, coordinates: ['54.23215','33.3333'], icon: '2x' };
  ...
  it('addWeather should update layer3 with weather template', () => {
    const objectWeather: IWeatherObject = { city: 'Moscow', temperature: 5, coordinates: ['54.23215','33.3333'], icon: '2x' };
    
    view.addWeatherTemplate = jest.fn().mockReturnValue('<div>Moscow</div><div>5</div>');
    view.addWeather(objectWeather);
    
    expect(mockElement.querySelector('.layer3')!.innerHTML).toBe('<div>Moscow</div><div>5</div>');
  });
  ...
  it('updateMapLink should update map image source', () => {
    ...
    expect(img.src).toContain('https://static-maps.yandex.ru/v1?lang=ru_RU&ll=37.6173,55.7558');
  });
